...
@section('content')
    <div class="max-w-4xl mx-auto bg-white shadow-xl rounded-xl overflow-hidden border border-gray-100">
        <div class="px-8 py-6 bg-gray-50 border-b border-gray-200">
            ...
            <p class="text-sm text-gray-500 mt-1">Fill in the details to create a new lesson for your students.</p>
        </div>

        <form action="{{ route('lessons.store') }}" method="POST" enctype="multipart/form-data" class="p-8 space-y-8">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div class="space-y-2">
                    <label for="estimated_duration" class="block text-sm font-semibold text-gray-700">Duration (mins)</label>
            ...
            <div class="space-y-2">
                ...
                <textarea name="description" id="description" rows="3"
                    class="w-full rounded-lg border-gray-300 bg-white px-4 py-3 text-gray-700 shadow-sm focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 focus:outline-none transition-all duration-200" placeholder="Brief overview of the lesson scope..."></textarea>
            </div>
            ...
            <div x-data="{ tab: 'file' }">
                <div class="flex items-center justify-between mb-6">
                     <h3 class="text-lg font-bold text-gray-800">Lesson Content</h3>
                ...
                <!-- Text Input -->
                <div x-show="tab === 'text'" class="bg-gray-50 p-6 rounded-xl border border-gray-200 transition-all duration-300">
                    <label for="content_text" class="block text-sm font-semibold text-gray-700 mb-2">Detailed Content</label>
                    <textarea name="content_text" id="content_text" rows="12"
                        class="block w-full rounded-lg border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 focus:outline-none sm:text-sm transition-all duration-200"
                        placeholder="# Chapter 1: Introduction..."></textarea>
                </div>

                <!-- File Upload -->
                <div x-show="tab === 'file'" class="bg-indigo-50 p-6 rounded-xl border-2 border-dashed border-indigo-200 hover:border-indigo-300 transition-all duration-300">
                    <label for="lesson_file" class="block text-sm font-semibold text-gray-700 mb-4">Upload File (TXT, PDF, DOCX)</label>
                    <div class="flex flex-col items-center justify-center pt-8 pb-8 bg-white rounded-lg cursor-pointer hover:bg-gray-50 transition-colors relative">
                        <svg class="w-14 h-14 text-indigo-400 mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"></path></svg>
                        <div class="flex text-sm text-gray-600">
                            <label for="lesson_file_input" class="relative cursor-pointer bg-white rounded-md font-medium text-indigo-600 hover:text-indigo-500 focus-within:outline-none">
